...
public class Sales extends javax.swing.JPanel {
    ...
    private void loadSales() {
        try {
            ResultSet resultSet = MySql.execute("SELECT * FROM `sales`");
            Vector fv = new Vector();
            fv.add("SELECT");

            while (resultSet.next()) {
                fv.add(resultSet.getString("sa_name"));
                salesMap.put(resultSet.getString("sa_name"), resultSet.getInt("sa_id"));
            }
    ...
    private void loadSalesDetails() {
        try {
            ResultSet resultSet = MySql.execute("SELECT * FROM `sales_details` INNER JOIN `sales` ON `sales_details`.`sales_sa_id` = `sales`.`sa_id`");
            ...
            while (resultSet.next()) {
                ...
                pv.add(resultSet.getString("sad_id"));
                pv.add(resultSet.getString("sa_name"));
    ...
    private void clearSalesRegister() {
        jTextField3.setText("");
        jTextField1.setText("");
        loadSales();
    }
    ...
    private void initComponents() {
        ...
        jLabel6.setText("Sales Register");
    ...
    private void jButton2ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton2ActionPerformed
        String sales = String.valueOf(jComboBox1.getSelectedItem());
        Double amount = Double.valueOf(jFormattedTextField1.getText());

        if ("SELECT".equals(sales)) {
            JOptionPane.showMessageDialog(this, "Please Selete Sales", "Warning", JOptionPane.WARNING_MESSAGE);
        } else if (amount == 0.0) {
            JOptionPane.showMessageDialog(this, "Please Enter Amount", "Warning", JOptionPane.WARNING_MESSAGE);
        } else {
            int salesId = salesMap.get(sales);

            LocalDate d = LocalDate.now();
            DateTimeFormatter f = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            String date = d.format(f);
            try {
                MySql.execute("INSERT INTO `sales_details` (`date`,`amount`,`sales_sa_id`) VALUES('" + date + "','" + amount + "','" + salesId + "')");
                clearSales();
                loadSalesDetails();
            } catch (Exception e) {
                e.printStackTrace();
    ...
    private void jButton3ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButton3ActionPerformed
        ...
        if (name.isEmpty()) {
            ...
            try {
                ResultSet resultSet = MySql.execute("SELECT * FROM `sales` WHERE `sa_name` = '" + name + "' OR `sa_code` = '" + code + "'");
                if (!resultSet.next()) {
                    MySql.execute("INSERT INTO `sales` (`sa_code`,`sa_name`)VALUES('" + code + "','" + name + "')");
                    loadSales();
                    clearSalesRegister();
                } else {
